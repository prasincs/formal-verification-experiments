...
impl Keyboard {
    ...
    /// `crate::usb::UsbKeyboard` (behind the `usb` feature), which drives the
    /// DWC2 host controller and feeds each 8-byte report to
    /// [`process_hid_report`](Self::process_hid_report).
